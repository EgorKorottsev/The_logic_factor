 ...
 */
public class EasyQuestionsOpenHelper extends SQLiteOpenHelper {
    private static final String INT = " INTEGER, ";

    public EasyQuestionsOpenHelper(Context context) {
        super(context, "database_easy_questions.db", null, 1);
    }


    @Override
    public void onCreate(SQLiteDatabase database) {
        database.execSQL("create table table_easy_questions " +
    ...
    public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
        database.execSQL("DROP TABLE IF EXISTS table_easy_questions");
        onCreate(database);
    }
}
